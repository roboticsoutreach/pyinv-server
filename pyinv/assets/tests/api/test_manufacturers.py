...
class TestManufacturerListEndpoint(ManufacturerTestCase):
    ...
    def test_single_result(self, api_client: TestClient, manufacturer: Manufacturer) -> None:
        data = self._subject(api_client)
        assert data["count"] == 1
        assert data["next"] is None
        assert data["previous"] is None
    ...
    @pytest.mark.usefixtures("manufacturer", "manufacturer_alt")
    def test_order_by_slug_asc(self, api_client: TestClient) -> None:
        data = self._subject(api_client, params={"ordering": "slug"})
        assert [d["name"] for d in data["results"]] == ["Foo", "Bar"]

    @pytest.mark.usefixtures("manufacturer", "manufacturer_alt")
    def test_order_by_slug_desc(self, api_client: TestClient) -> None:
        data = self._subject(api_client, params={"ordering": "-slug"})
        assert [d["name"] for d in data["results"]] == ["Bar", "Foo"]
...
class TestManufacturerPostEndpoint(ManufacturerTestCase):
    ...
    def test_post(self, user_client: TestClient, user: User) -> None:
    ...
    def test_post_default_slug(self, user_client: TestClient, user: User) -> None:
        data = {"name": "Bar"}
        self._set_permission(user)
        resp = user_client.post(self._subject, data)
        assert resp.status_code == 201
    ...
    def test_post_bad_slug(self, user_client: TestClient, user: User) -> None:
    ...
    def test_post_duplicate_slug(self, user_client: TestClient, user: User) -> None:
...
class TestManufacturerGetIndividualEndpoint(ManufacturerTestCase):
    ...
    def test_fetch_manufacturer(self, api_client: TestClient, manufacturer: Manufacturer) -> None:
        resp = api_client.get(f"{self._subject}/foo/")
...
class TestManufacturerPutIndividualEndpoint(ManufacturerTestCase):
    ...
    def test_put_manufacturer(self, user_client: TestClient, user: User) -> None:
        self._set_permission(user)
    ...
    def test_put_manufacturer_no_auth(self, api_client: TestClient) -> None:
        resp = api_client.put(self._subject)
        assert resp.status_code == 403
        assert resp.json() == {'detail': 'Authentication credentials were not provided.'}

    def test_put_manufacturer_no_permissions(self, user_client: TestClient) -> None:
        resp = user_client.put(self._subject)
        assert resp.status_code == 403
        assert resp.json() == {'detail': 'You do not have permission to perform this action.'}

    def test_put_manufacturer_timestamps(self, user_client: TestClient, user: User) -> None:
        self._set_permission(user)
...
class TestManufacturerPatchIndividualEndpoint(ManufacturerTestCase):
    ...
    def test_patch_manufacturer(self, user_client: TestClient, user: User) -> None:
        self._set_permission(user)
    ...
    def test_patch_manufacturer_no_auth(self, api_client: TestClient) -> None:
        resp = api_client.patch(self._subject)
        assert resp.status_code == 403
        assert resp.json() == {'detail': 'Authentication credentials were not provided.'}

    def test_patch_manufacturer_no_permissions(self, user_client: TestClient) -> None:
        resp = user_client.patch(self._subject)
        assert resp.status_code == 403
        assert resp.json() == {'detail': 'You do not have permission to perform this action.'}

    def test_patch_manufacturer_timestamps(self, user_client: TestClient, user: User) -> None:
        self._set_permission(user)
...
class TestManufacturerDeleteIndividualEndpoint(ManufacturerTestCase):
    ...
    def test_delete_manufacturer(self, user_client: TestClient, user: User) -> None:
        self._set_permission(user)
    ...
    def test_delete_manufacturer_with_model(self, user_client: TestClient, user: User) -> None:
        self._set_permission(user)
        resp = user_client.delete(self._subject)
        assert resp.status_code == 400
        assert resp.json() == {'detail': 'Unable to delete object because it is referenced by other objects.'}

    def test_delete_manufacturer_no_auth(self, api_client: TestClient) -> None:
        resp = api_client.delete(self._subject)
        assert resp.status_code == 403
        assert resp.json() == {'detail': 'Authentication credentials were not provided.'}

    def test_delete_manufacturer_no_permissions(self, user_client: TestClient) -> None:
        resp = user_client.delete(self._subject)
        assert resp.status_code == 403
        assert resp.json() == {'detail': 'You do not have permission to perform this action.'}
